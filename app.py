...
from wtforms.fields.html5 import DateField  # type: ignore
...
from tensorflow.keras.layers import ZeroPadding2D, Convolution2D, MaxPooling2D  # type: ignore
from tensorflow.keras.layers import Dropout, Softmax, Flatten, Activation  # type: ignore

app = Flask(__name__)
load_dotenv()
# app.config['MYSQL_HOST'] = 'host.docker.internal'
app.config['MYSQL_HOST'] = '127.0.0.1'
app.config['MYSQL_USER'] = 'proctor'
...
app.config['S3_BUCKET'] = 'bucket name'
app.config['S3_KEY'] = 'S3_KEY'
app.config['S3_SECRET'] = 'S3_SECRET/S3_SECRET'
app.config['S3_LOCATION'] = f"http://{app.config['S3_BUCKET']}.s3.amazonaws.com/"
app.config['S3_REGION'] = 'ap-south-1'

app.config['MAIL_SERVER'] = 'smtpout.MAIL_SERVER.net'
app.config['MAIL_PORT'] = 465
app.config['MAIL_USERNAME'] = 'admin@MAIL_USERNAME.com'
...
app.config["CSV_ALLOWED_EXTENSIONS"] = {'csv'}

app.config["PHOTO_ALLOWED_EXTENSIONS"] = {'png', 'jpg', 'jpeg', 'gif'}
...
def download_image_on_server(url, uid):
    response = requests.get(url, stream=True)
    response.raise_for_status()

    image_path = os.path.join(app.config['UPLOAD_FOLDER'], str(uid) + '.png')
    with open(image_path, 'wb') as out_file:
        shutil.copyfileobj(response.raw, out_file)


def upload_image_on_server(imgdata1, uid):
    if imgdata1.startswith("data:image/png;base64,"):
        imgdata1 = imgdata1[len("data:image/png;base64,"):]

    image_data = base64.b64decode(imgdata1)
    image_path = os.path.join(app.config['UPLOAD_FOLDER'], str(uid) + '.png')
    with open(image_path, 'wb') as f:
        f.write(image_data)


def convert_image_to_base64(uid):
    file_path = 'webBanner/proctor/'
...
def window_event():
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        results = cur.execute('INSERT INTO window_estimation_log (email, test_id, name, window_event, uid) values(%s,%s,%s,%s,%s)', (dict(
            session)['email'], testid, dict(session)['name'], 1, dict(session)['uid']))
        mysql.connection.commit()
        cur.close()
        if (results > 0):
    ...
    if results > 0:
        ...
        for a in cresults:
            if datetime.strptime(str(a['start']), "%Y-%m-%d %H:%M:%S") <= now and datetime.strptime(str(a['end']), "%Y-%m-%d %H:%M:%S") >= now:
                testids.append({'tid': a['tid'], 'name': a['test_id']})
        cur.close()
...
def payment():
    cur = mysql.connection.cursor()
    cur.execute('SELECT examcredits FROM USERS where email = %s and uid = %s',
                (session['email'], session['uid']))
...
def report_professor_email():
    if request.method == 'POST':
        ...
        msg1 = Message('PROBLEM REPORTED', sender=sender,
                       recipients=[careEmail])
        msg1.body = " ".join(
            ["NAME:", cname, "PROBLEM TYPE:", ptype, "EMAIL:", cemail, "", "QUERY:", cquery])
        mail.send(msg1)
...
def contact():
    if request.method == 'POST':
        careEmail = "youranimebuddy001@gmail.com"
        ...
        msg2 = Message('Hello', sender=sender, recipients=[careEmail])
        msg2.body = " ".join(
            ["NAME:", cname, "EMAIL:", cemail, "QUERY:", cquery])
        mail.send(msg1)
        mail.send(msg2)
...
def lostpassword():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        results = cur.execute(
            'SELECT * from users where email = %s', [lpemail])
        if results > 0:
            sesOTPfp = generateOTP()
            session['tempOTPfp'] = sesOTPfp
            session['seslpemail'] = lpemail
            msg1 = Message('MyProctor.ai - OTP Verification for Lost Password',
                           sender=sender, recipients=[lpemail])
    ...
    if request.method == 'POST':
        ...
        if (npwd == cpwd):
            hashed_password = bcrypt.hashpw(
                npwd.encode('utf-8'), bcrypt.gensalt())
            cur = mysql.connection.cursor()
            cur.execute('UPDATE users set password = %s where email = %s',
                        (hashed_password, slpemail))
            mysql.connection.commit()
            cur.close()
            session.clear()
...
# upload image to s3 bucket function


def upload_image_on_server(image_data, user_id):
    s3 = boto3.client(
        's3',
        aws_access_key_id=app.config['S3_KEY'],
        aws_secret_access_key=app.config['S3_SECRET'],
    ...
    file_path = f'webBanner/proctor/{user_id}.png'
    s3.put_object(
    ...
    return f'https://{app.config["S3_BUCKET"]}.s3.{app.config["S3_REGION"]}.amazonaws.com/{file_path}'
...
def register():
    if request.method == 'POST':
        ...
        msg1 = Message('MyProctor.ai - OTP Verification',
...
def login():
    if request.method == 'POST':
        ...
        results1 = cur.execute(
            'SELECT uid, name, email, password, user_type, user_image from users where email = %s and user_type = %s and user_login = 0', (email, user_type))
        if results1 > 0:
            ...
            imgdata2 = convert_image_to_base64(uid)
            ...
            path_to_weights = 'models/deep_face/vgg_face_weights.h5'
            model.load_weights(path_to_weights)

            # model_weights = load_model(path_to_weights)
            img_result = DeepFace.verify(
                image1, image2, model_name="VGG-Face", model=model)
            # img_result  = DeepFace.verify(image1, image2, enforce_detection = False)

            if img_result["verified"] == True and check_password(password_candidate, password):
                results2 = cur.execute(
                    'UPDATE users set user_login = 1 where email = %s', [email])
                mysql.connection.commit()
                if results2 > 0:
    ...
    if request.method == 'POST':
        ...
        if theOTP == mOTP:
            ...
            ar = cur.execute('INSERT INTO users(name, email, password, user_type, user_image, user_login) VALUES (%s, %s, %s, %s, %s, %s)',
                             (dbName, dbEmail, dbPassword, dbUser_type, '', 0))
            mysql.connection.commit()
            if ar > 0:
                uid = cur.lastrowid
                image_url = upload_image_on_server(dbImgdata, uid)
                cur.execute(
                    'UPDATE users SET user_image=%s WHERE uid=%s', (image_url, uid))
                mysql.connection.commit()
...
def changePassword():
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        results = cur.execute(
            'SELECT * from users where email = %s and uid = %s', (session['email'], session['uid']))
        if results > 0:
            data = cur.fetchone()
...
def subject():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        if subject_id:
            cur.execute('UPDATE subject SET subject_name = %s, description = %s WHERE subject_id = %s',
                        (subject_name, description, subject_id))
            flash('Subject updated successfully', 'success')
        else:
            cur.execute('INSERT INTO subject (subject_name, description) values(%s,%s)',
                        (subject_name, description))
            flash('Subject added successfully', 'success')
        mysql.connection.commit()
        return redirect(url_for('subject'))
    cur = mysql.connection.cursor()
    results = cur.execute('select * from subject')
    results = cur.fetchall()
    subject_dict = {}
    for result in results:
        subject_dict[result['subject_id']] = {
            'subject_name': result['subject_name'], 'description': result['description']}
    return render_template('subject.html', subjects=results, subject_string_data=json.dumps(subject_dict))
...
@app.route('/import_subject', methods=['POST'])
@user_role_professor
def import_subject():
    if request.method == 'POST':
        ...
        if not file or file.filename == '' or not allowed_file(file.filename, app.config['CSV_ALLOWED_EXTENSIONS']):
            flash('No selected file', 'error')
            return redirect(url_for('subject'))

        filename = secure_filename(file.filename)
        file.stream.seek(0)
        ef = pd.read_csv(file.stream)
        fields = ['subject_name', 'description']
        df = pd.DataFrame(ef, columns=fields)
        cur = mysql.connection.cursor()
        for row in df.index:
            cur.execute('INSERT INTO subject(subject_name,description) values(%s,%s)',
                        (df['subject_name'][row], df['description'][row]))
...
def topic():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        if topic_id:
            cur.execute('UPDATE topic SET topic_name = %s, description = %s, subject_id = %s WHERE topic_id = %s',
                        (topic_name, description, subject_id, topic_id))
            flash('Topic updated successfully', 'success')
        else:
            cur.execute('INSERT INTO topic (topic_name, description, subject_id) VALUES (%s,%s,%s)',
    ...
    subject_dict = {}
    for topic in topics:
        topic_dict[topic['topic_id']] = {'topic_name': topic['topic_name'],
                                         'description': topic['description'], 'subject_id': topic['subject_id']}
    for subject in subjects:
        subject_dict[subject['subject_id']] = {
            'subject_name': subject['subject_name']}
    return render_template('topic.html', topics=topics, subjects=subject_dict, topic_string_data=json.dumps(topic_dict), subject_string_data=json.dumps(subject_dict))


@app.route("/import_topic", methods=["POST"])
@user_role_professor
def import_topic():
    if request.method == "POST":
        if "doc" not in request.files:
        ...
        file.stream.seek(0)
        ef = pd.read_csv(file.stream)
        fields = ["topic_name", "subject_name", "description"]
        df = pd.DataFrame(ef, columns=fields)
        ...
        for index, row in df.iterrows():
            ...
            if not subject:
                flash(
                    f"Subject '{row['subject_name']}' does not exist, skipping entry",
                    "warning",
                )
                continue
            ...
            cur.execute(
                "INSERT INTO topic (topic_name, subject_id, description) VALUES (%s, %s, %s)",
                (row["topic_name"], subject["subject_id"], row["description"]),
            )
            mysql.connection.commit()

        cur.close()
        flash("Topics imported successfully", "success")
        return redirect(url_for("topic"))
...
def user():
    if request.method == 'POST':
        ...
        if not uid and not password:
            flash('Password are required', 'error')
            return redirect(url_for('user'))

        if not name or not email or not user_type:
            flash('Name, Email and User Type are required', 'error')
            return redirect(url_for('user'))

        cur = mysql.connection.cursor()
        if uid:
            # Update user
            if password and user_image:
                cur.execute('UPDATE users SET name = %s, email = %s, password = %s, user_type = %s, user_image = %s WHERE uid = %s',
                            (name, email, hashed_password, user_type, user_image, uid))
            elif password:
                cur.execute('UPDATE users SET name = %s, email = %s, password = %s, user_type = %s WHERE uid = %s',
                            (name, email, hashed_password, user_type, uid))
            elif user_image:
                cur.execute('UPDATE users SET name = %s, email = %s, user_type = %s, user_image = %s WHERE uid = %s',
                            (name, email, user_type, user_image, uid))
            else:
                cur.execute(
                    'UPDATE users SET name = %s, email = %s, user_type = %s WHERE uid = %s', (name, email, user_type, uid))
            flash('User updated successfully', 'success')
        else:
            cur.execute('INSERT INTO users (name, email, password, user_type, user_image) VALUES (%s, %s, %s, %s, %s)',
                        (name, email, hashed_password, user_type, user_image))
            uid = cur.lastrowid
            flash('User added successfully', 'success')

        if 'user_image' in request.files:
            file = request.files['user_image']
            if file and allowed_file(file.filename, app.config['PHOTO_ALLOWED_EXTENSIONS']):
                file.save(os.path.join(
                    app.config['UPLOAD_FOLDER'], str(uid) + ".png"))
    ...
    users = cur.fetchall()
    user_dict = {user['uid']: {'name': user['name'], 'email': user['email'], 'user_type': user['user_type'], 'user_image': user['user_image']}
                 for user in users}
    return render_template('user.html', users=users, user_string_data=json.dumps(user_dict))
...
def import_users():
    ...
    with open(os.path.join(app.config['UPLOAD_FOLDER'], filename)) as csvfile:
        ...
        for row in csv_reader:
            ...
            user_type = 'proctor' if row['user_type'] == 'proctor' else "student"
            # user_image_url = row.get('user_image')

            if not name:
                flash('Name is required for row ' + str(i), 'error')
                continue
            if not email:
                flash('Email is required for row ' + str(i), 'error')
                continue
            if not plain_password:
                flash('Password is required for row ' + str(i), 'error')
                continue
            # if not user_image_url:
                # flash('Image Url is required for row ' + str(i), 'error')
                # continue

            password = bcrypt.hashpw(
                plain_password.encode('utf-8'), bcrypt.gensalt())
            cur.execute('INSERT INTO users (name, email, password, user_type, user_image,user_login) VALUES (%s, %s, %s, %s, 1,%s)',
                        (name, email, password, user_type, user_login))

            # Upload Image on Server
            # download_image_on_server(user_image_url, cur.lastrowid)
...
class PracUploadForm(FlaskForm):
    ...
    compiler = SelectField(u'Compiler/Interpreter', choices=[('11', 'C'), ('27', 'C#'), ('1', 'C++'), ('114', 'Go'), ('10', 'Java'), ('47', 'Kotlin'), ('56', 'Node.js'),
                                                             ('43', 'Objective-C'), ('29', 'PHP'), ('54', 'Perl-6'), ('116', 'Python 3x'), ('117', 'R'), (
                                                                 '17', 'Ruby'), ('93', 'Rust'), ('52', 'SQLite-queries'), ('40', 'SQLite-schema'),
                                                             ('39', 'Scala'), ('85', 'Swift'), ('57', 'TypeScript')])
    password = PasswordField(
        'Exam Password', [validators.Length(min=3, max=10)])
    proctor_type = RadioField('Proctoring Type', choices=[(
        '0', 'Automatic Monitoring'), ('1', 'Live Monitoring')])
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if ecc:
            ...
            cur.execute('INSERT INTO practicalqa(test_id,qid,q,compiler,marks,uid) values(%s,%s,%s,%s,%s,%s)',
                        (test_id, 1, questionprac, compiler, marksprac, session['uid']))
            mysql.connection.commit()
            start_date = form.start_date.data
            end_date = form.end_date.data
            start_time = form.start_time.data
...
def deldispques():
    if request.method == 'POST':
        ...
        elif et['test_type'] == "full":
            cur = mysql.connection.cursor()
            cur.execute('SELECT * from allquestions where test_id = %s and uid = %s',
                        (tidoption, session['uid']))
            callresults = cur.fetchall()
            cur.close()
            return render_template("deldisallquestions.html", callresults=callresults, tid=tidoption)
        else:
        ...
        if request.method == 'POST':
            ...
            if testqdel:
                if ',' in testqdel:
                    testqdel = testqdel.split(',')
                    for getid in testqdel:
                        cur.execute('DELETE FROM allquestions WHERE test_id = %s and qid =%s and uid = %s', (
                            testid, getid, session['uid']))
                        mysql.connection.commit()
                    resp = jsonify(
                        '<span style=\'color:green;\'>Questions deleted successfully</span>')
...
def del_qid(testid, qid):
    cur = mysql.connection.cursor()
    results = cur.execute(
        'DELETE FROM questions where test_id = %s and qid = %s and uid = %s', (testid, qid, session['uid']))
    mysql.connection.commit()
    if results > 0:
...
def updatedispques():
    if request.method == 'POST':
        ...
        if et['test_type'] == "objective":
            ...
            callresults = cur.fetchall()
            cur.close()
            return render_template("updatedispquesAQ.html", callresults=callresults)
        else:
            flash('Error Occured!')
...
def update_lqa(testid, qid):
    if request.method == 'GET':
        cur = mysql.connection.cursor()
        cur.execute('SELECT * FROM longqa where test_id = %s and qid =%s and uid = %s',
                    (testid, qid, session['uid']))
        uresults = cur.fetchall()
        mysql.connection.commit()
...
@app.route('/updateAQ/<testid>/<qid>', methods=['GET', 'POST'])
@user_role_professor
def updateAQ(testid, qid):
    if request.method == 'GET':
        cur = mysql.connection.cursor()
        cur.execute('SELECT * FROM allquestions where test_id = %s and qid =%s and uid = %s',
                    (testid, qid, session['uid']))
        uresults = cur.fetchall()
        mysql.connection.commit()
        return render_template("updateQuestionsAQ.html", uresults=uresults)
    if request.method == 'POST':
        ques = request.form['ques']
        markso = request.form['mko']
        cur = mysql.connection.cursor()
        cur.execute('UPDATE allquestions SET q = %s, marks = %s where test_id = %s and qid = %s and uid = %s',
                    (ques, markso, testid, qid, session['uid']))
        cur.connection.commit()
        flash('Updated successfully.', 'success')
        cur.close()
...
def update_PQA(testid, qid):
    if request.method == 'GET':
        cur = mysql.connection.cursor()
        cur.execute('SELECT * FROM practicalqa where test_id = %s and qid =%s and uid = %s',
                    (testid, qid, session['uid']))
        uresults = cur.fetchall()
        mysql.connection.commit()
...
def displayquestions():
    if request.method == 'POST':
        ...
        if et['test_type'] == "objective":
            ...
            callresults = cur.fetchall()
            cur.close()
            return render_template("displayallquestion.html", callresults=callresults)
...
def viewstudentslogs():
    cur = mysql.connection.cursor()
    results = cur.execute(
        'SELECT test_id from teachers where email = %s and uid = %s and proctoring_type = 0', (session['email'], session['uid']))
    if results > 0:
        cresults = cur.fetchall()
        cur.close()
...
def insertmarkstid():
    cur = mysql.connection.cursor()
    results = cur.execute('SELECT * from teachers where show_ans = 0 and email = %s and uid = %s and (test_type = %s or test_type = %s)',
    ...
    if results > 0:
        ...
        for a in cresults:
            if datetime.strptime(str(a['end']), "%Y-%m-%d %H:%M:%S") <= now:
                testids.append(a['test_id'])
        cur.close()
        return render_template("insertmarkstid.html", cresults=testids, res=results, nw=now)
    else:
...
def displaystudentsdetails():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            'SELECT DISTINCT email,test_id from proctoring_log where test_id = %s', [tidoption])
        callresults = cur.fetchall()
        cur.close()
...
def insertmarksdetails():
    if request.method == 'POST':
        ...
        if et['test_type'] == "subjective":
            cur = mysql.connection.cursor()
            cur.execute(
                'SELECT DISTINCT email,test_id from longtest where test_id = %s', [tidoption])
            callresults = cur.fetchall()
            cur.close()
            ...
            cur = mysql.connection.cursor()
            cur.execute(
                'SELECT DISTINCT email,test_id from practicaltest where test_id = %s', [tidoption])
            callresults = cur.fetchall()
            cur.close()
...
def insertsubmarks(testid, email):
    ...
    if request.method == "POST":
        cur = mysql.connection.cursor()
        results1 = cur.execute(
            'SELECT COUNT(qid) from longtest where test_id = %s and email = %s', (testid, email))
...
def insertpracmarks(testid, email):
    ...
    if request.method == "POST":
        cur = mysql.connection.cursor()
        results1 = cur.execute(
            'SELECT COUNT(qid) from practicaltest where test_id = %s and email = %s', (testid, email))
        results1 = cur.fetchone()
        cur.close()
        for sa in range(1, results1['COUNT(qid)']+1):
            marksByProfessor = request.form[str(sa)]
...
def displaywinstudentslogs(testid, email):
    cur = mysql.connection.cursor()
    cur.execute(
        'SELECT * from window_estimation_log where test_id = %s and email = %s and window_event = 1', (testid, email))
    callresults = cur.fetchall()
    cur.close()
    return callresults
...
def countTotalstudentslogs(testid, email):
    cur = mysql.connection.cursor()
    cur.execute(
        'SELECT COUNT(*) as total from proctoring_log where test_id = %s and email = %s', (testid, email))
    callresults = cur.fetchall()
    cur.close()
...
def share_details(testid, email):
    cur = mysql.connection.cursor()
    cur.execute(
        'SELECT * from teachers where test_id = %s and email = %s', (testid, email))
    callresults = cur.fetchall()
    cur.close()
...
def share_details_emails():
    if request.method == 'POST':
        ...
        msg1 = Message('EXAM DETAILS - MyProctor.ai',
                       sender=sender, recipients=[emailssharelist])
        msg1.body = " ".join(["EXAM-ID:", tid, "SUBJECT:", subject, "TOPIC:", topic, "DURATION:", duration, "START",
                             start, "END", end, "PASSWORD", password, "NEGATIVE MARKS in %:", neg_marks, "CALCULATOR ALLOWED:", calc])
        mail.send(msg1)
...
def viewresults():
    if request.method == 'POST':
        ...
        if et['test_type'] == "subjective":
            cur = mysql.connection.cursor()
            cur.execute(
                'SELECT SUM(marks) as marks, email from longtest where test_id = %s group by email', ([tidoption]))
            callresults = cur.fetchall()
            cur.close()
            ...
            cur = mysql.connection.cursor()
            cur.execute(
                'SELECT SUM(marks) as marks, email from practicaltest where test_id = %s group by email', ([tidoption]))
            callresults = cur.fetchall()
            cur.close()
            return render_template("publish_viewresults.html", callresults=callresults, tid=tidoption)
        else:
...
def publish_results():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            'UPDATE teachers set show_ans = 1 where test_id = %s', ([tidoption]))
        mysql.connection.commit()
        cur.close()
...
def test_update_time():
    if request.method == 'POST':
        ...
        testid = request.form['testid']
        cur.execute('UPDATE studenttestinfo set time_left=SEC_TO_TIME(%s) where test_id = %s and email = %s and uid = %s and completed=0',
...
def give_test():
    ...
    if request.method == 'POST' and form.validate():
        ...
        results1 = cur1.execute(
            'SELECT uid, user_image from users where email = %s and user_type = %s ', (session['email'], 'student'))
        if results1 > 0:
            cresults = cur1.fetchone()
            # imgdata2 = cresults['user_image']
            imgdata2 = convert_image_to_base64(cresults['uid'])
            cur1.close()
            nparr1 = np.frombuffer(base64.b64decode(imgdata1), np.uint8)
            nparr2 = np.frombuffer(base64.b64decode(imgdata2), np.uint8)
            image1 = cv2.imdecode(nparr1, cv2.COLOR_BGR2GRAY)
            ...
            if img_result["verified"] == True:
                cur = mysql.connection.cursor()
                results = cur.execute(
                    'SELECT * from teachers where test_id = %s', [test_id])
                if results > 0:
                    data = cur.fetchone()
                    password = data['password']
                    ...
                    if password == password_candidate:
                        ...
                        if datetime.strptime(start, "%Y-%m-%d %H:%M:%S") < now and datetime.strptime(end, "%Y-%m-%d %H:%M:%S") > now:
                            results = cur.execute(
                                'SELECT time_to_sec(time_left) as time_left,completed from studenttestinfo where email = %s and test_id = %s', (session['email'], test_id))
                            if results > 0:
                                ...
                                if is_completed == 0:
                                    ...
                                    if time_left <= duration:
                                        duration = time_left
                                        results = cur.execute('SELECT qid , ans from students where email = %s and test_id = %s and uid = %s', (
                                ...
                                mysql.connection.commit()
                                results = cur.execute('SELECT time_to_sec(time_left) as time_left,completed from studenttestinfo where email = %s and test_id = %s and uid = %s', (
                                    session['email'], test_id, session['uid']))
                                if results > 0:
                                    results = cur.fetchone()
                                    is_completed = results['completed']
                                    if is_completed == 0:
                                        ...
                                        if time_left <= duration:
                                            duration = time_left
                                            results = cur.execute(
                                                'SELECT * from students where email = %s and test_id = %s and uid = %s', (session['email'], test_id, session['uid']))
                                            marked_ans = {}
                                            if results > 0:
                                                results = cur.fetchall()
                                                for row in results:
    ...
    if callresults['test_type'] == "objective":
        ...
        if request.method == 'GET':
            ...
            if flag == 'get':
                ...
                cur = mysql.connection.cursor()
                results = cur.execute(
                    'SELECT * from students where test_id =%s and qid = %s and email = %s', (testid, qid, session['email']))
                if results > 0:
                ...
                cur = mysql.connection.cursor()
                cur.execute('UPDATE studenttestinfo set completed=1,time_left=sec_to_time(0) where test_id = %s and email = %s and uid = %s',
        ...
        if request.method == 'GET':
            cur = mysql.connection.cursor()
            cur.execute(
                'SELECT test_id, qid, q, marks from longqa where test_id = %s ORDER BY RAND()', [testid])
            callresults1 = cur.fetchall()
            cur.execute('SELECT time_to_sec(time_left) as duration from studenttestinfo where completed = 0 and test_id = %s and email = %s and uid = %s',
                        (testid, session['email'], session['uid']))
            studenttestinfo = cur.fetchone()
            if studenttestinfo != None:
                duration = studenttestinfo['duration']
                cur.execute(
                    'SELECT tid,test_id, subject, topic, proctoring_type from teachers where test_id = %s', [testid])
                ...
                cur.close()
                return render_template("testsubjective.html", callresults=callresults1, subject=subject, duration=duration, test_id=test_id, topic=topic, proctortypes=proctortypes, tid=tid)
            else:
                cur = mysql.connection.cursor()
                cur.execute(
                    'SELECT test_id, duration, subject, topic from teachers where test_id = %s', [testid])
                testDetails = cur.fetchone()
                subject = testDetails['subject']
            ...
            cur = mysql.connection.cursor()
            results1 = cur.execute(
                'SELECT COUNT(qid) from longqa where test_id = %s', [testid])
            results1 = cur.fetchone()
            cur.close()
            ...
            for sa in range(1, results1['COUNT(qid)']+1):
                ...
                cur = mysql.connection.cursor()
                insertStudentData = cur.execute('INSERT INTO longtest(email,test_id,qid,ans,uid) values(%s,%s,%s,%s,%s)', (
                    session['email'], testid, sa, answerByStudent, session['uid']))
                mysql.connection.commit()
            else:
                if insertStudentData > 0:
                    insertstudenttestinfoData = cur.execute(
                        'UPDATE studenttestinfo set completed = 1 where test_id = %s and email = %s and uid = %s', (test_id, session['email'], session['uid']))
                    mysql.connection.commit()
                    cur.close()
        ...
        if request.method == 'GET':
            cur = mysql.connection.cursor()
            cur.execute(
                'SELECT test_id, qid, q, marks, compiler from practicalqa where test_id = %s ORDER BY RAND()', [testid])
            callresults1 = cur.fetchall()
            cur.execute('SELECT time_to_sec(time_left) as duration from studenttestinfo where completed = 0 and test_id = %s and email = %s and uid = %s',
                        (testid, session['email'], session['uid']))
            studenttestinfo = cur.fetchone()
            if studenttestinfo != None:
                duration = studenttestinfo['duration']
                cur.execute(
                    'SELECT test_id, subject, topic, proctoring_type from teachers where test_id = %s', [testid])
                testDetails = cur.fetchone()
                subject = testDetails['subject']
                test_id = testDetails['test_id']
                topic = testDetails['topic']
            ...
            cur = mysql.connection.cursor()
            insertStudentData = cur.execute('INSERT INTO practicaltest(email,test_id,qid,code,input,executed,uid) values(%s,%s,%s,%s,%s,%s,%s)', (
...
def check_result(email, testid):
    if email == session['email']:
        cur = mysql.connection.cursor()
        results = cur.execute(
            'SELECT * FROM teachers where test_id = %s', [testid])
        if results > 0:
            results = cur.fetchone()
...
def totmarks(email, tests):
    ...
    for test in tests:
        testid = test['test_id']
        results = cur.execute(
            "select neg_marks from teachers where test_id=%s", [testid])
        results = cur.fetchone()
...
def tests_given(email):
    if request.method == "GET":
        if email == session['email']:
            ...
            resultsTestids = cur.execute(
                'select studenttestinfo.test_id as test_id from studenttestinfo,teachers where studenttestinfo.email = %s and studenttestinfo.uid = %s and studenttestinfo.completed=1 and teachers.test_id = studenttestinfo.test_id and teachers.show_ans = 1 ', (session['email'], session['uid']))
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            'SELECT test_type from teachers where test_id = %s', [tidoption])
        callresults = cur.fetchone()
        cur.close()
        if callresults['test_type'] == "objective":
            cur = mysql.connection.cursor()
            ...
            for a in results:
                results1.append(
                    neg_marks(a['email'], a['test_id'], a['neg_marks']))
...
def tests_created(email):
    if email == session['email']:
        cur = mysql.connection.cursor()
        results = cur.execute(
            'select * from teachers where email = %s and uid = %s and show_ans = 1', (email, session['uid']))
        results = cur.fetchall()
    ...
    if email == session['email']:
        ...
        if request.method == 'GET':
            if et['test_type'] == "objective":
                cur = mysql.connection.cursor()
                results = cur.execute(
                    'select users.name as name,users.email as email, studenttestinfo.test_id as test_id from studenttestinfo, users where test_id = %s and completed = 1 and  users.user_type = %s and studenttestinfo.email=users.email ', (testid, 'student'))
                results = cur.fetchall()
                cur.close()
                final = []
                names = []
                ...
                cur = mysql.connection.cursor()
                results = cur.execute(
                    'select users.name as name,users.email as email, longtest.test_id as test_id, SUM(longtest.marks) AS marks from longtest, users where longtest.test_id = %s  and  users.user_type = %s and longtest.email=users.email', (testid, 'student'))
                results = cur.fetchall()
                cur.close()
                names = []
                scores = []
...
def disptests(email):
    if email == session['email']:
        cur = mysql.connection.cursor()
        results = cur.execute(
            'select * from teachers where email = %s and uid = %s', (email, session['uid']))
        results = cur.fetchall()
...
@user_role_professor
def create_test_single():
    if request.method == 'POST':
       questions_data = zip(request.form.getlist('question[]'),
                             request.form.getlist('option_a[]'),
                             request.form.getlist('option_b[]'),
                             request.form.getlist('option_c[]'),
                             request.form.getlist('option_d[]'),
                             request.form.getlist('answer[]'),
                             request.form.getlist('marks[]'))
       cur = mysql.connection.cursor()
       test_id = generate_slug(2)
       for index, (ques, opt_a, opt_b, opt_c, opt_d, ans, marks) in enumerate(questions_data, start=1):
            qid = f'{index}'
            cur.execute('INSERT INTO allquestions(test_id, qid, q, a, b, c, d, ans, marks, uid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (
                        test_id, qid, ques, opt_a, opt_b, opt_c, opt_d, ans, marks, session['uid']))
            mysql.connection.commit()
       ...
       subject = request.form['subject']
       topic = request.form['topic']
       proctor_type = request.form['proctor_type']
       cur.execute('INSERT INTO teachers (email, test_id, test_type, start, end, duration, show_ans, password, subject, topic, neg_marks, calc, proctoring_type, uid) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
                (dict(session)['email'], test_id, "full", start_date_time, end_date_time, duration, 0, password, subject, topic, 0, 0, proctor_type, session['uid']))
       mysql.connection.commit()
       cur.execute('UPDATE users SET examcredits = examcredits-1 where email = %s and uid = %s',
                   (session['email'], session['uid']))
       mysql.connection.commit()
